...
export default function EditVehicle(){
  ...
  useEffect(()=> {
    const saved = JSON.parse(localStorage.getItem("mv_edit_payload") || "{}");
    setPayload(p => ({...p, ...saved}));
  },[]);

  useEffect(()=> localStorage.setItem("mv_edit_payload", JSON.stringify(payload)), [payload]);
  ...
  const submit = async () => {
    ...
    try {
      const res = await axios.post("http://localhost:3001/proxy/topaz/multiVehicle/edit", payload, { headers });
      setResp(res.data);
    } catch(e){
  ...
  return (
    <div style={{padding:20, maxWidth:900, margin:"auto"}}>
      ...
      <div style={{marginTop:12}}>
        <button onClick={submit} disabled={loading}>{loading?"Processing...":"Submit Edit"}</button>
      </div>
